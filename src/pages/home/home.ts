...
export class HomePage {
  ...
  onSearch(ev: any){
    let val:string = this.removeDiacritics(ev.target.value);
    val = val.toLowerCase();

    if(val!="") {
      this.clearSearch();

      this.recetteList.forEach(recette => {
        let nom:string = this.removeDiacritics(recette.nom);
        nom = nom.toLowerCase();
        
        if(nom.search(val)!=-1) this.resultlist.push(recette);
        else {
          if(recette.motsCles!=undefined){
            for(let i=0 ; i<recette.motsCles.length ; i++){
              let motCle:string = this.removeDiacritics(recette.motsCles[i]);
              motCle = motCle.toLowerCase();

              if(motCle.search(val)!=-1){
                this.resultlist.push(recette);
                break;
              }
            }
          }
  ...
  removeDiacritics (str) {
    let changes = this.defaultDiacriticsRemovalMap;
